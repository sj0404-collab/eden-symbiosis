...
class EnginesFragment : Fragment() {
    ...
    private fun render() {
        ...
        root.addView(
            note(
                "Основное ядро — Symbiosis, оно в APK и запускает игры. " +
                    "Второе ядро скачивается отдельно (это Kenji-NX под нашей маркой, ~55 МБ). " +
                    "В этот APK его нельзя ужать до размера Eden: это другой эмулятор. " +
                    "Пока второе ядро не подключено как плеер, запуск игр остаётся на Symbiosis."
            )
        )
