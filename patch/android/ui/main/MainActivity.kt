...
class MainActivity : AppCompatActivity(), ThemeProvider {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!checkedDecryption) {
            // Always check: there is no wizard left to have completed first.
            checkKeys()
    ...
    private fun setUpNavigation(navController: NavController) {
        // The setup wizard is gone. It asked for keys, firmware and a games
        // folder before the user had any of them to hand, then declared itself
        // "Done!" whether or not anything had been set up. The same actions are
        // available from Tools, at the moment they are actually wanted, and the
        // status strip on the home screen says what is still missing.
        //
        // Nothing is configured by default either: no folder is invented, so
        // the app cannot claim a folder was "already added" that the user never
        // chose.
    }
