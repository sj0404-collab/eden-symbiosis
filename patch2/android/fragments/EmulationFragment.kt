...
import org.yuzu.yuzu_emu.views.FloatingGameButton
...
class EmulationFragment : Fragment(), SurfaceHolder.Callback {
    ...
    // A thumb-sized button floating over the picture. Every action on it
    // leaves emulation running - the drawer's pause entry is deliberately not
    // offered. Checked against upstream: opening the drawer does not pause by
    // itself, only R.id.menu_pause_emulation does.
    private var floatingButton: FloatingGameButton? = null
    ...
    private fun attachFloatingButton() {
        ...
        val button = FloatingGameButton(requireContext())
        button.attach(
            binding.emulationContainer,
            FloatingGameButton.Actions(
                openMenu = {
                    // Opens the existing drawer. This does not pause: upstream
                    // pauses only from the menu entry, never on open.
                    if (_binding != null) binding.drawerLayout.open()
                },
                toggleControls = {
                    if (_binding != null) {
