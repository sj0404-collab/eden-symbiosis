...
class EmulationActivity : AppCompatActivity(), SensorEventListener, InputManager.InputDeviceListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ── Keeping other apps alive ────────────────────────────────
        //
        // ForegroundService exists, in upstream's own words, "to avoid the
        // app getting cleared from memory by the system". It works by raising
        // this process's priority - and the memory it keeps has to come from
        // somewhere, so Android evicts whatever else was running. That is why
        // the browser and the chat reload after a session.
        //
        // The service is only needed while a game is actually emulating: it
        // is what stops Android killing the emulator when the screen is off
        // or another app comes forward. It is NOT needed for the overlay
        // editor, which draws controls with nothing running behind them.
        //
        // So: skip it entirely in overlay-edit mode, and let the user turn it
        // off for gameplay too. Off means other apps survive; the trade is
        // that a long spell in the background can have the game killed. The
        // default stays ON, exactly as upstream behaves, because changing
        // what happens by default would be a surprise.
        val keepAlivePrefs =
            PreferenceManager.getDefaultSharedPreferences(YuzuApplication.appContext)
        val keepAlive = keepAlivePrefs.getBoolean(PREF_KEEP_IN_MEMORY, true)
        val editingOverlayOnly = intent.getBooleanExtra(EXTRA_OVERLAY_GAMELESS_EDIT_MODE, false)
        if (keepAlive && !editingOverlayOnly) {
    ...
    companion object {
        ...
        /**
         * Whether to hold the emulator in memory while a game runs.
         *
         * Plain SharedPreferences, deliberately not a Setting: settings go
         * through NativeConfig into config.ini, which is the emulator's own
         * configuration file, and this is a property of the Android app
         * rather than of the emulated machine. It also means the value is
         * readable before the core is up.
         *
         * Default true - unchanged upstream behaviour.
         */
        const val PREF_KEEP_IN_MEMORY = "symbiosis_keep_in_memory"
        ...
        fun stopForegroundService(activity: Activity) {
            // startForegroundService STARTS the service if it is not running -
            // even when the intent says "stop". The service then has a few
            // seconds to call startForeground() or Android kills the process
            // with ForegroundServiceDidNotStartInTimeException.
            //
            // Upstream never hit this because the service was always running
            // by the time anything stopped it. Now that it can be switched
            // off, and is skipped in overlay-edit mode, "stop" can arrive
            // when nothing was ever started - so the plain stopService is
            // used, which does nothing at all when there is nothing to stop.
            val stopIntent = Intent(activity, ForegroundService::class.java)
            stopIntent.action = ForegroundService.ACTION_STOP
            runCatching { activity.startService(stopIntent) }
            runCatching { activity.stopService(Intent(activity, ForegroundService::class.java)) }
        }
