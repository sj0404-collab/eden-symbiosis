...
import org.yuzu.yuzu_emu.utils.CoreFromFolder
...
object NativeLibrary {
    ...
    /**
     * Why the core is not always inside the APK.
     *
     * libyuzu-android.so is 83 MB of compiled C++ - 19.6 MB once packed - and
     * it almost never changes: a typical change here is a Kotlin file and a
     * layout, not a single .cpp. Shipping it every time means re-downloading
     * the same unchanged 20 MB to move a button.
     *
     * So: use the built-in copy when there is one, and otherwise take it from
     * a folder the user placed on the device once. CoreFromFolder handles the
     * part that makes this non-obvious - external storage is mounted noexec,
     * so the library must be copied into app storage before it can be loaded
     * at all, and since Android 14 it must be read-only when it is.
     *
     * The failure is deliberately loud. An emulator with no core cannot do
     * anything, and a silent failure here shows up later as an unexplained
